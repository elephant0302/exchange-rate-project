...
- 7일·30일 통계 예측(랜덤워크 벤치마크, 수익률 AR, Holt, 달러 팩터, 결합 + GARCH 구간)
...
- 단기 환율의 표준 벤치마크는 Meese-Rogoff(1983) 랜덤워크(RW)입니다. 7~30일 구간에서는 거시 모델이 RW를 이기기 어렵다는 결과가 반복됩니다(Rossi 2013).
- 이 앱은 API 키 없이 재현 가능한 표준 시계열 스택만 씁니다.
  - RW: 직전 종가 유지
  - AR: 로그수익률 AR(p), AIC로 시차 선택
  - Holt: 가법 추세 지수평활(ETS, Hyndman류)
  - DollarFactor: 유로·엔·위안에 대해 `r_t = a + b r_USD,t-1` (시차 달러 팩터)
  - Combination: Bates-Granger 역MAE 가중 결합
  - 구간: 수익률 GARCH(1,1) 누적 변동성, 검증 잔차와 더 넓은 쪽
- UIP·PPP·테일러 준칙은 금리·물가 시계열이 필요해서 키 없는 현재 공급자로는 넣지 않았습니다. 가짜 거시 데이터로 채우지 않습니다.
- 최근 3주를 7일씩 걸어가며 검증합니다. 결과는 중심값과 95% 근사 구간이며 확정 시세가 아닙니다.
